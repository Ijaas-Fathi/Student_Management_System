document.addEventListener('DOMContentLoaded', () => {   const studentForm = document.getElementById("studentForm");   const studentTable = document     .getElementById("studentTable") 
    .getElementsByTagName("tbody")[0]; 
   const apiBaseUrl = "http://localhost:8080/api/students"; 
   function fetchStudent() {     fetch(apiBaseUrl) 
  ...
  };    window.deleteEmployee = function (id) {     fetch(`${apiBaseUrl}/${id}`, {       
    method: "DELETE", 
    }).then(() => fetchEmployees()); 
  };    fetchEmployees(); 
}); 
